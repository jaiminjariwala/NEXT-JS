...
const ColorChanger: FC<ColorChangerProps> = ({ slice }) => {
  ...
  return (
    <section
      ...
      className="relative flex h-[90vh] min-h-[1000px] flex-col overflow-hidden bg-white text-black"
    >
      {/* SVG background */}
      <svg
      ...
      >
        <text
          ...
          className="fill-black/10 uppercase group-hover:fill-black/20 motion-safe:transition-all motion-safe:duration-700"
        >
      ...
      {/* Canvas */}
      <Canvas
        camera={{ position: [0, 0.5, 0.5], fov: 35, zoom: 2.12 }}
      ...
      <Bounded
        ...
        innerClassName="gap-6 lg:gap-8 flex flex-col lg:fex-row"
      >
          ...
          {KEYCAP_TEXTURES.map((texture) => (
            <li key={texture.id}>
              <button
                ...
                className={clsx(
                  "flex aspect-square flex-col items-center justify-center rounded-lg p-4 hover:scale-105 motion-safe:transition-all motion-safe:duration-300",
