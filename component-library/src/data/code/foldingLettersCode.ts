export const foldingLettersCode = `'use client';
...
const MAX_PAGES = 5;
const ACTIVE_TILT_OPTIONS = [-5, -4, -3, 3, 4, 5];

const getTopSegmentClipPath = (flattenProgress = 0) => {
  const inset = TOP_SEGMENT_EDGE_INSET * (1 - flattenProgress);
  return 'polygon(' + inset + '% 0%, ' + (100 - inset) + '% 0%, 100% 100%, 0% 100%)';
};

const getBottomSegmentTransform = (fold, offsetY, offsetZ) =>
...
const LETTERS = [
  {
    ...
    signature: 'Dominik',
    body: "Thank you for this note and for the work behind it. I've already spent some time with the new screensaver, and I genuinely find it enchanting. It really stands out in the best possible way. In a web full of visual noise and disposable design, this feels calm, intentional, and remarkably refined.",
  },
  {
    id: 2,
    greeting: 'Hey Atilla,',
    signature: 'Jeff',
    body: 'Just want to say this is an incredible release. Starting with the background on the installer disk image, it is clear you are about to install software that was not developed, but painstakingly crafted. I was so very thrilled to see Slate and Noir. The whole experience feels considered from edge to edge, and that restraint gives it a rare kind of confidence.',
  },
  {
    id: 3,
    greeting: 'Hi Atilla,',
    signature: 'Chris',
    body: 'The new clock is amazing. And I loved the old version too. Thank you for your beautiful work. It makes me happy to look at it every day. You can sense immediately that someone cared deeply about proportions, color, and depth. That level of care is what makes it feel calm instead of decorative.',
  },
  {
    id: 4,
    greeting: 'Hey Atilla,',
    signature: 'Martin',
    body: 'My favorite clock screensaver just got a major glow-up. Bauhaus Clock 2.0. Rebuilt from the ground up, insanely precise, and visually on another level. You can really feel the upgrade. Putting this level of effort into details is genuinely admirable.',
  },
  {
    id: 5,
    greeting: 'Hi Atilla,',
    signature: 'Roobhir',
    body: 'While I did a double-take at the $19 price tag for a screensaver, it was an easy decision after the first couple of seconds. An excellently crafted product clearly made with lots of love and attention to detail. It feels premium in a way that is immediately visible and difficult to fake.',
  },
];

const PAGES = LETTERS.slice(0, MAX_PAGES);

function FullLetterLayout({ letter, isHidden }) {
  return (
    <div
      className={
        'pointer-events-none flex h-[540px] w-full select-none flex-col box-border px-11 py-10 text-[#1d1d1f] transition-opacity duration-300 ' +
...
const FoldingCard = forwardRef(function FoldingCard(
  {
    letter,
    state = 'idle',
    onNavigate,
  ...
  const getInitialStyles = () => {
    ...
    return {
      transform:
        'translate3d(0, ' +
        ACTIVE_CARD_OFFSET_Y +
        'px, 0) scale(1) rotateX(0deg) rotateZ(' +
        activeTilt +
        'deg)',
      fold: INITIAL_FOLD,
  ...
  useEffect(() => {
    if (state !== 'entering' || !wrapRef.current) return;
    ...
    mainAnimation.onfinish = () => {
      ...
      if (midRef.current) midRef.current.style.transform = 'rotateX(' + -INITIAL_FOLD + 'deg)';
      ...
      midAnimation && midAnimation.cancel();
      bottomAnimation && bottomAnimation.cancel();
      topAnimation && topAnimation.cancel();
      onNavigate('entered');
    };
  ...
  const handleGlobalMove = useCallback(
    (event) => {
      ...
      if (midRef.current) midRef.current.style.transform = 'rotateX(' + middleFold + 'deg)';
      if (bottomRef.current) {
        bottomRef.current.style.transform = getBottomSegmentTransform(
          bottomFold,
          bottomOffsetY,
  ...
  const handleGlobalEnd = useCallback(() => {
    ...
    if (mainAnimation) {
      mainAnimation.onfinish = () => {
        ...
        if (topRef.current) topRef.current.style.transform = 'rotateX(' + INITIAL_FOLD + 'deg)';
        if (topRef.current) topRef.current.style.clipPath = getTopSegmentClipPath();
      };
    }
  }, [activeTilt, handleGlobalMove, triggerExit]);

  const handleStart = (event) => {
    if (!isInteractive || stateRef.current === 'entering') return;
...
function ResetCascade({ letters, onDone }) {
  useEffect(() => {
    let cancelled = false;
        ...
        return (
          <div
            ...
            style={{
              animation:
                'resetSlideStraight ' +
                RESET_CASCADE_DURATION +
                'ms cubic-bezier(0.45, 0, 0.55, 1) ' +
                index * RESET_CASCADE_STAGGER +
                'ms forwards',
...
export default function FoldingLetters() {
  const [leftStack, setLeftStack] = useState([...PAGES.slice(1)].reverse());
  ...
  const [outgoingTransition, setOutgoingTransition] = useState(null);
  ...
  const handleNavigate = useCallback(
    (result) => {
      if (result === 'entered') {
        setIsBusy(false);
        setActiveLetter((previous) => (previous ? { ...previous, state: 'active' } : null));
        return;
      }
      ...
      if (result === 'right') {
        setRightStack((previous) => previous.concat(currentLetter));

        if (leftStack.length === 0) {
          setActiveLetter(null);
          setTimeout(() => setIsResetting(true), 150);
          return;
        }
      ...
      setLeftStack((previous) => previous.concat(currentLetter));
  ...
  useEffect(() => {
    ...
    const frameId = window.requestAnimationFrame(() => {
      outgoingRef.current && outgoingRef.current.exit(outgoingTransition.exitSide);
    });

    return () => window.cancelAnimationFrame(frameId);
  }, [outgoingTransition]);

  const handleOutgoingNavigate = useCallback((result) => {
    if (result === 'entered') return;

    setOutgoingTransition((current) => {
      if (!current || current.exitSide !== result) return current;
  ...
  const startButtonTransition = useCallback(
    (side) => {
      if (!activeLetter || isBusy || isResetting) return;
    ...
    >
      ...
      <div className="pointer-events-none absolute inset-x-0 top-[46%] z-[400] flex -translate-y-1/2 justify-between px-10">
        <button
          onClick={handlePrev}
          className={
            'pointer-events-auto flex h-12 w-12 items-center justify-center rounded-full border border-black/5 bg-white/90 shadow-[0_12px_24px_rgba(15,23,42,0.08),0_2px_6px_rgba(15,23,42,0.05)] backdrop-blur transition-all ' +
            (rightStack.length === 0 || isResetting || isBusy
              ? 'cursor-not-allowed opacity-20'
              : 'hover:scale-110 hover:shadow-md active:scale-95')
          }
        >
          <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#222" strokeWidth="2.5">
      ...
      <div className="card-wrap absolute left-1/2 top-[46%] h-[540px] w-[420px] origin-center [transform:translate(-50%,-50%)_scale(0.92)]">
        ...
        {isResetting && <ResetCascade letters={[...PAGES].reverse()} onDone={finishReset} />}
        ...
        {!isResetting && activeLetter && (
          <FoldingCard
            ...
            pileSide={activeLetter.from}
            onNavigate={handleNavigate}
            currentStackHeight={activeLetter.from === 'left' ? rightStack.length : leftStack.length}
            sourceStackHeight={activeLetter.from === 'left' ? leftStack.length : rightStack.length}
          />
        )}
      </div>
    </div>
