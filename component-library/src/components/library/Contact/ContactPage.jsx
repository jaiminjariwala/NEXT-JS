...
function AddFileHint({ addFileBtnRef, sentinelRef }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      await _hhAnim(cursor, [{ opacity: 0 }, { opacity: 1 }], { duration: 200, fill: "forwards" });
      if (deadRef.current) return;
      await _hhAnim(cursor, _hhArcFrames(P0, P1, P2, 8), { duration: 900, easing: "ease-in-out", fill: "forwards" });
      if (deadRef.current) return;
      await _hhAnim(
        cursor,
  ...
  return (
    <>
      ...
      {showTip && (
        <div
          aria-hidden="true"
          style={{ position: "fixed", top: pos.tty, left: pos.ttx, zIndex: 9999, pointerEvents: "none", whiteSpace: "nowrap", animation: "hh-in 0.2s ease forwards" }}
    ...
    if (rawText.includes("\n")) {
      ...
      requestAnimationFrame(() =>
        requestAnimationFrame(() => {
          ...
          if (lastEl) {
            setCaretOffset(lastEl, (extraLines[extraLines.length - 1] ?? "").length);
      ...
      if (offset === 0 && i > 0) {
        ...
        requestAnimationFrame(() =>
          requestAnimationFrame(() => {
            ...
            savedAfter.forEach((t, idx) => {
              if (lineRefs.current[i + idx]) lineRefs.current[i + idx].innerText = t;
            });
            updateMessageFromDOM();
          })
        );
    ...
    const processed = await Promise.all(
      files.map(async (file, index) => {
        ...
        return {
          ...
          file: finalFile,
          isImage,
          previewUrl: isImage ? URL.createObjectURL(finalFile) : "",
        };
      })
    );
          ...
          >
            ...
            <div className="grid grid-cols-[80px_1fr_80px] items-center bg-transparent px-[8px] py-[8px] md:grid-cols-[120px_1fr_120px] md:px-[12px] md:py-[12px]">
              <button
                ...
                onClick={onCancel}
                disabled={sending}
                className="h-auto rounded-[7px] border border-[#c9cfda] bg-[#f9f9f8] px-[8px] py-[7px] text-[17px] font-semibold leading-none tracking-[-0.01em] text-[#000000] shadow-[inset_0_1px_0_rgba(255,255,255,0.92),inset_0_-1px_0_rgba(164,174,188,0.32)] transition-transform active:scale-[0.98] disabled:opacity-60 md:h-auto md:rounded-[8px] md:px-[14px] md:py-[9px] md:text-[20px] md:font-semibold md:shadow-[inset_0_1px_0_rgba(255,255,255,0.92),inset_0_-1px_0_rgba(164,174,188,0.32)]"
              >
                Cancel
              </button>
              ...
              <button
                ...
                onClick={onSend}
                disabled={sending}
                className="h-auto rounded-[7px] border border-[#6f97d9] bg-gradient-to-b from-[#8FC0FF] via-[#5C9CF4] to-[#2F72E2] px-[8px] py-[7px] text-[17px] font-semibold text-[#ffffff] leading-none shadow-[inset_0_1px_0_rgba(255,255,255,0.62),inset_0_-1px_0_rgba(25,67,154,0.45),0_1px_1px_rgba(29,72,157,0.28)] transition-transform active:scale-[0.98] disabled:opacity-60 md:h-auto md:rounded-[8px] md:px-[14px] md:py-[9px] md:text-[20px] md:font-semibold"
              >
                {sending ? (
                  <SendingDots />
                ) : sent ? (
            ...
            >
              ...
              <input
                ...
                className={`cp-from-input flex-1 appearance-none border-0 bg-transparent tracking-[-0.01em] outline-none ring-0 focus:outline-none focus:ring-0 ${
                  fromError && !from.trim() ? "placeholder:text-[#d53030]" : "placeholder:text-[#a1a8b3]"
                }`}
                style={{
                  border: "none",
                  boxShadow: "none",
                ...
                {Array.from({ length: lineCount }, (_, i) => (
                  <div
                    ...
                    style={{
                      ...
                      backgroundColor:
                        allMessageSelected && (message.split("\n")[i] ?? "").trim()
            ...
            {/* ── Add file bar ── */}
            <div
              className="relative flex items-center justify-start overflow-hidden rounded-b-[8px] bg-transparent px-[10px] py-[7px] md:rounded-b-[16px] md:px-[12px] md:py-[9px]"
              style={{
