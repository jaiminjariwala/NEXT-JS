...
export const FlipCalendar = () => {
  ...
  useEffect(() => {
    ...
    const startAutoPlay = () => {
      let count = 0;
      const flipTypes = ['next', 'prev', 'next', 'prev']; // Demo sequence

      const runDemo = () => {
        if (count < flipTypes.length && isAutoPlay) {
          handleFlip(flipTypes[count], true);
          count++;
          autoPlayTimerRef.current = setTimeout(runDemo, 2500); // Wait for animation + delay
        } else {
          // Loop the demo
          count = 0;
    ...
    <div className="flex flex-col items-center justify-center p-8">
      ...
      >
        ...
        <div className="relative w-full h-full bg-blue-600 rounded-[3.5rem] overflow-hidden shadow-2xl flex flex-col">
          ...
          {/* TOP HALF BACKGROUND - Fades when flipping NEXT, reveals TARGET when flipping PREV */}
          <div 
          ...
          {/* BOTTOM HALF BACKGROUND - Fades when flipping PREV, reveals TARGET when flipping NEXT */}
          <div 
          ...
          {/* THE MOVING LEAF - NEXT (Lifts from bottom to top) */}
          <div 
          ...
          {/* THE MOVING LEAF - PREV (Drops from top to bottom) */}
          <div 
            className={`absolute inset-x-0 top-0 h-1/2 bg-blue-600 rounded-t-[3.5rem] origin-bottom overflow-hidden
